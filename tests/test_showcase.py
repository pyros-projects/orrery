...
def test_every_showcase_preset_runs_clean_across_seeds(home, name):
    ...
    assert screenplay == (name.split("/")[0] in ("h3", "effects", "curator") or "h3" in preset_meta(h, name)["tags"])
...
@pytest.mark.parametrize("name", [n for n in SHOWCASE if n.split("/")[0] in ("effects", "curator")])
