...
| **3. Galaxy v0** | A single local HTML page over `galaxy.jsonl` + thumbnails: filter by pick, rate with love/like/nope/hate, ratings update the learned-weight file that layer 1 reads. Every card offers **copy template** (resolved from its `#hash`), **copy exact prompt**, **copy seed** and **save as preset** (with folder and tags). A preset browser by folder and tag. The wildcard manager lives here as a UI over layer 1's operations. | 100 real images rated, the next batch visibly shifts toward loved regions, and a loved output's template has been saved as a preset from its card. |
